...
- Implementar banco de dados:
    - Substituir armazenamento temporário em memória por **persistência em arquivos**.
    - Utilizar **Files** (da API `java.nio.file`) para operações simples (como verificar existência, criar arquivos, ler linhas).
    - Utilizar **InputStream e OutputStream** (`FileInputStream`, `FileOutputStream`, `BufferedReader`, `BufferedWriter`) para manipular dados binários ou textuais.
    - Garantir que veículos, clientes e aluguéis sejam gravados e recuperados de arquivos.
- Implementar **serialização binária** de objetos com `ObjectOutputStream` e `ObjectInputStream`.
- Criar **lambda factories** usando `Supplier` para gerar veículos de teste ou clientes fictícios.
